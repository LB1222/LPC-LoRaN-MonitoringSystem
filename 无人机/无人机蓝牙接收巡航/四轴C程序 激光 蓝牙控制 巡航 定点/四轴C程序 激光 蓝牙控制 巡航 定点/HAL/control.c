...
int16_t  HIGH_START =150;   //一键起飞目标高度
...
//				{
//					set_high = 1;
//					pidHeightHigh.desired = Control_high;//记录高度当做当前定高高度		  
//				}
...
void Flow_mode_two(void)
{
		...
		if((mini.ok == 1) && (Control_high>5) && (Control_high<400))//判断是否存在光流  高度高于5CM 光流才可以定点
		{
	...
	switch(mode_flag)
	{
		case 0:		//巡航
		{
			...
			{  		
				...
				move_X=0;	
				move_X=0;	
				Remote.thr = 	Remote.thr-50;   //通道3 油门通道在原来的基础上自动慢慢减小  直到飞机慢慢下降		 
			}
			else
			if(SStime>20)	
			{	      
				Flow_PosPid_x.desired = 0;//记录位置 //刷新位置期望
				Flow_PosPid_y.desired = 0;//记录位置 //刷新位置期望
			}
			else
			if(SStime>15)	
			{	      
				Flow_PosPid_x.desired = 50;//记录位置 //刷新位置期望
				Flow_PosPid_y.desired = 0;//记录位置 //刷新位置期望
			}
			else
			if(SStime>10)	
			{	      
				Flow_PosPid_x.desired = 50;//记录位置 //刷新位置期望
				Flow_PosPid_y.desired = 50;//记录位置 //刷新位置期望
			}
			else
			if(SStime>5)	
			{
...
//				move_X=0;	
//				move_X=0;	
//				Remote.thr = 	Remote.thr-50;   //通道3 油门通道在原来的基础上自动慢慢减小  起到飞机慢慢下降		 
//			}
//			else
//			if(SStime>15)	
//			{	      
...
//			if(SStime>5)	
//			{
//				Flow_PosPid_x.desired = 100;//记录位置 //刷新位置期望
//				Flow_PosPid_y.desired = 50;//记录位置 //刷新位置期望
//			}
//			else
//			if(SStime>0)
...
void Flow_Pos_Controler(float dt)  //位置定点控制器  光流
{
  ...
  if((mini.ok == 1) && (Control_high>5) && (Control_high<400))//判断是否存在光流  高度高于20CM 光流才可以定点
	{	
