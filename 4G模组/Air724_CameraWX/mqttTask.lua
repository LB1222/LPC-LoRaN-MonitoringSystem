    ...
    function()
        ...
        while true do
            ...
            if socket.isReady() then
                ...
                local subscribe_topic = "user/"..imei;
                --创建一个MQTT客户端
               --local mqttClient = mqtt.client(imei,600,"","")
                local mqttClient = mqtt.client(imei,600,"yang","11223344")
               ...
               -- if mqttClient:connect("broker.emqx.io",1883,"tcp") then
              if mqttClient:connect("mnif.cn",1883,"tcp") then
                    retryConnectCnt = 0
                    ready = true
                    --订阅主题
                    if mqttClient:subscribe(subscribe_topic,0) then
